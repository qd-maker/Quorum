"""Supabase 历史记录服务（带用户隔离）— 异步版本.

使用 AsyncClient + httpx.AsyncClient 连接池复用，
...
async def _get_db() -> AsyncClient:
    ...
    if _db_client is None:
        s = get_settings()
        _db_client = await acreate_client(s.SUPABASE_URL, s.SUPABASE_SERVICE_KEY)
    return _db_client
...
async def create_session(
    ...
    res = await db.table("sessions").insert(data).execute()
    return res.data[0]
...
async def get_session(session_id: str, user_id: str) -> dict | None:
    """获取单个会话（校验归属权）."""
    db = await _get_db()
    res = await db.table("sessions").select("*") \
        .eq("id", session_id).eq("user_id", user_id).execute()
...
async def replace_messages(session_id: str, messages: list[dict]) -> None:
    """全量替换会话消息（先删后插）."""
    db = await _get_db()
    await db.table("messages").delete().eq("session_id", session_id).execute()
    if not messages:
        return
    rows = []
    for msg in messages:
